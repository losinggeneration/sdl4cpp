...
namespace SDL
{
	 ...
	 *
	 * A Rect defines a rectangular area of pixels. It is used by
	 * SDL::Surface::Blit() to define blitting regions and by several other
	 * video functions.
	 *
	...
	class Rect : public SDL_Rect
	{
		public:
			 ...
			 *
			 * \return The Rect that is the subtraction of all values of the
			 * Rect by one.
			 */
			Rect &operator --();
	 ...
	 *
	 * This (read-only) structure is returned by Screen::GetVideoInfo(). It
	 * contains information on either the 'best' available mode (if called
	 * before Screen::SetVideoMode()) or the current video mode.
	 *
	 * \b Structure Definition
	 * \code
	 * typedef struct
	 ...
	 * [RGBA]shift	Binary left shift of each color component in the pixel
	 * value
	 ...
	 * surfaces, or assign the values of one surface to another.
	 */
	class Surface
	{
		public:
			 ...
			 * something to it before calling anything else that works on it.
			 * Otherwise it will throw an SDL::LogicError
			 */
			Surface();
			 ...
			 *
			 * \throws SDL::RuntimeError if blitting to this surface from the
			 * copy fails.
			 * \throws SDL::LogicError if the surface's m_Surface is NULL
			 */
			 ...
			 *
			 * Creates a surface of width w, height h, bits per pixel bpp, and
			 * flags
			 *
			 ...
			 * surface.
			 * \throws SDL::RuntimeError if the surface passed is NULL
			 */
			 ...
			 *
			 * Compare all "public" members of m_Surface including the values
			 * of pixel data.
			 *
			 * \return True if they have the same values in them, False
			 * otherwise.
			 */
			 ...
			 *
			 * \throws SDL::LogicError if either Surface::m_Surface is NULL.
			 */
			 ...
			 *
			 * The aruments are reversed here beause I think this will be less
			 * used than the function to blit to a destrect.
			 *
			 * \return True if the blit was sucessfull. otherwise false.
			 *
			 * \throws SDL::LogicError if either Surface::m_Surface is NULL.
			 ...
			 *
			 * This is the strangest one to me because I don't know why the
			 * other Blits wouldn't be called instead.
			 *
			 * So this shouldn't be needed but kept just in case someone wants
			 * it.
			 *
			 * \return True if the blit was sucessfull. otherwise false.
			 *
			 * \throws SDL::LogicError if either Surface::m_Surface is NULL.
			 */
			friend bool Blit(Rect &srcrect, const Surface &src, Rect &destrect,
							 Surface &dest);
			 ...
			 * Documention not written yet.
			 */
			int SetPalette(int flags, Color &colors, int firstcolor,
						   int ncolors);
			 ...
			 *
			 * This shouldn't normally be used, but is here to accomodate
			 * things I may have forgotten, overlooked, etc. \b Use \b with
			 * \b care.
			 */
			SDL_Surface *Get();
			 ...
			 *
			 * Frees the resources used by a previously created from m_Surface.
			 * If the surface was created using CreateRGBSurfaceFrom() then the
			 * pixel data is not freed.
			 *
			 * \note There isn't any functions to check for
			 * CreateRGBSurfaceFrom(), so for now you should take care of that.
			 */
			void Free();
		protected:
			/*!
			 ...
			 *
			 * SDL_Su5rface's represent areas of "graphical" memory, memory
			 * that can be drawn to. The video framebuffer stored in Screen
			 * from Screen::SetVideoMode() and this SDL_Surface can be accessed
			 * via Screen::GetVideoSurface(). Most of the fields should be
			 * pretty obvious. w and h are the width and height of the surface
			 * in pixels. pixels is a pointer to the actual pixel data, the
			 * surface should be locked before accessing this field. The
			 * clip_rect field is the clipping rectangle as set by
			 * SetClipRect().
			 *
	...
	namespace GL
	{
		 ...
		 *
		 * While you can set most OpenGL attributes normally, the attributes
		 * listed above must be known before SDL sets the video mode. These
		 * attributes are set and read with SetAttribute() and GetAttribute().
		 *
		 * \htmlonly
		 * <pre>
		 ...
		 * be done before Screen::SetVideoMode() is called. The path of the GL
		 * library is passed to LoadLibrary(). You must then use
		 * GetProcAddress() to retrieve function pointers to GL functions.
		 *
		 * \return true an sucess and false on an error.
		 * \throws SDL::LogicError if the Screen has been initialized before
		 * calling.
		 */
		 ...
		 *
		 * Returns the address of the GL function proc, or NULL if the function
		 * is not found. If the GL library is loaded at runtime, with
		 * LoadLibrary(), then all GL functions must be retrieved this way.
		 * Usually this is used to retrieve function pointers to OpenGL
		 * extensions. Note that this function needs an OpenGL context to
		 * function properly, so it should be called after
		 ...
		 * is not found.
		 * \throws SDL::LogicError if the Screen has not been initialized.
		 */
		 ...
		 * is useful after a call to Screen::SetVideoMode() to check whether
		 * your attributes have been set as you expected.
		 *
		 * \return true an sucess and false on an error.
		 * \throws SDL::LogicError if the Screen has not been initialized.
		 */
		 ...
		 * don't take effect until after a call to Screen::SetVideoMode(). You
		 * should use GetAttribute() to check the values after a
		 * Screen::SetVideoMode() call, since the values obtained can differ
	 ...
	 *
	 * This class inherits all of Surface and then adds the functionality to
	 * set the video mode (SetVideoMode()), icon (SetIcon()), gamma
	 * (SetGamma()), and gammaramp. It also has UpdateRect(), UpdateRects(),
	 * and Flip() to use.
	 *
	...
	class Screen : public Surface
	{
		public:
			 ...
			 *
			 * Sets up the video mode with these paramaters by calling
			 * SetVideoSurface().
			 *
			 ...
			 *
			 * \throws SDL::LogicError if the Screen has already been setup or
			 * if icon.m_Surface is NULL.
			 */
			 ...
			 *
			 * The rectangle must be confined within the screen boundaries (no
			 * clipping is done).
			 *
			 * If 'x', 'y', 'w' and 'h' in rect are all 0, UpdateRect will
			 * update the entire screen.
			 *
			 * \note This function should not be called while 'screen' is
			 * locked.
			 */
			 ...
			 * clipping is done).
			 *
			 * If 'x', 'y', 'w' and 'h' are all 0, UpdateRect will update the
			 * entire screen.
			 *
			 ...
			 */
			static bool SetGamma(float redgamma, float greengamma,
								 float bluegamma);
			 ...
			 * component. Each table is an array of 256 Uint16 values,
			 ...
			 * color precision. You may pass NULL to any of the channels to
			 ...
			 * can usually request and bpp you want when setting the video mode
			 ...
			 * to SetVideoMode()
			 *
			 * \return 0 if the requested mode is not supported under any bit
			 ...
			 */
			static int VideoModeOK(int w, int h, int bpp,
								   Uint32 flags = SDL_SWSURFACE);

			/*!
			 * \brief Obtain the name of the video driver.
			 *
			 * The driver name is a simple one word identifier like "x11" or
			 * "windib".
			 *
			 * \return an empty string if video has not been initialised with
			 * Init() or a string with the driver name in it.
			 */
			 ...
			 *
			 *
			 * The mask is a bitmask that describes the shape of the icon. If
			 * mask is NULL, then the shape is determined by the colorkey of
			 * icon, if any, or makes the icon rectangular (no transparency)
			 * otherwise.
			 *
			 * If mask is non-NULL, it points to a bitmap with bits set where
			 * the corresponding pixel should be visible. The format of the
			 * bitmap is as follows: Scanlines come in the usual top-down
			 ...
			 *
			 *
			 * Multiple Screen classes can be used after the call to
			 * SetVideoMode() by calling GetVideoSurface().
			 * \code
			 * void SetScreen()
			 * {
	 ...
	 *
	 * This is for anyone who wants overlays. I've never used them, so if
	 * someone wants to expand upon this, then feel free to.
	 *
	...
	class Overlay
	{
		public:
			 ...
			 * Documentation not written yet.
			 */
			void Create(int width, int height, Uint32 format,
						SDL_Surface *display);
