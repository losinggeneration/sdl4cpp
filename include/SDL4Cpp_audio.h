...
namespace SDL
{
	 ...
	 *
	 * Because of many C-isms that the audio portion of SDL uses makes it hard to cleanly wrap it into a class. I will work on this if I get enough emails wanting this. If not, use SDL_mixer.
	 *
	...
	*/
	namespace Audio
	{
		 ...
		 * \param samples	When used with SDL_OpenAudio this refers to the size of the audio buffer in samples. A sample a chunk of audio data of the size specified in format mulitplied by the number of channels. When the SDL_AudioSpec is used with SDL_LoadWAV samples is set to 4096.
		 *                                 
		 ...
		 * 
		 * \param *buf  This points to the audio data that will be used in the conversion. It is both the source and the destination, which means the converted audio data overwrites the original data. It also means that the converted data may be larger than the original data (if you were converting from 8-bit to 16-bit, for instance), so you must ensure buf is large enough. See below.
		 * \param len This is the length of the original audio data in bytes.
		 * \param len_mult As explained above, the audio buffer needs to be big enough to store the converted data, which may be bigger than the original audio data. The length of buf should be len*len_mult.
		 * \param len_ratio When you have finished converting your audio data, you need to know how much of your audio buffer is valid. len*len_ratio is the size of the converted audio data in bytes. This is very similar to len_mult, however when the convert audio data is shorter than the original len_mult would be 1. len_ratio, on the other hand, would be a fractional number between 0 and 1.
		 * 
		 ...
		 * 
		 * \e desired->format The desired audio format (see SDL_AudioSpec)
		 * 
		 * \e desired->samples The desired size of the audio buffer in samples. This number should be a power of two, and may be adjusted by the audio driver to a value more suitable for the hardware. Good values seem to range between 512 and 8192 inclusive, depending on the application and CPU speed. Smaller values yield faster response time, but can lead to underflow if the application is doing heavy processing and cannot fill the audio buffer in time. A stereo sample consists of both right and left channels in LR ordering. Note that the number of samples is directly related to time by the following formula: ms = (samples*1000)/freq
		 * 
		 * \e desired->callback This should be set to a function that will be called when the audio device is ready for more data. It is passed a pointer to the audio buffer, and the length in bytes of the audio buffer. This function usually runs in a separate thread, and so you should protect data structures that it accesses by calling SDL_LockAudio and SDL_UnlockAudio in your code. The callback prototype is:
		 * 
		 * \e void \e callback(void \e *userdata, \e Uint8 \e *stream, \e int \e len); userdata is the pointer stored in userdata field of the SDL_AudioSpec. stream is a pointer to the audio buffer you want to fill with information and len is the length of the audio buffer in bytes.
		 * 
		 * \e desired->userdata This pointer is passed as the first parameter to the callback function.
		 * 
		 ...
		 *
		 * \return either SDL_AUDIO_STOPPED, SDL_AUDIO_PAUSED or SDL_AUDIO_PLAYING depending on the current audio state.
		 * \sa PauseAudio
		 */
		 ...
		 */
		AudioSpec *LoadWAV(std::string file, AudioSpec *spec, Uint8 **audio_buf, Uint32 *audio_len);
		/*!
		...
		/*!
		 * \brief Initializes a SDL_AudioCVT structure for conversion
		 *
